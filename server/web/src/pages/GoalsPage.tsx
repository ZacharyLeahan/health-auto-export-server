...
interface WeeklyGoals {
  ...
  realWorkoutCadence: {
    ...
    daysSinceRealWorkout: number | null;
    hoursSinceRealWorkout: number | null;
...
function ProgressBar({ value, target, met }: { value: number; target: number; met: boolean }) {
...
function LiftingGauge({
  ...
  const fillColor = stretchMet ? 'bg-cyan-400' : baselineMet ? 'bg-emerald-500' : 'bg-amber-500';
    ...
    <div className="mt-4 rounded-lg border border-zinc-800 bg-zinc-950/60 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <p className="text-3xl font-semibold tabular-nums text-zinc-100">
          {value}
          <span className="text-base font-medium text-zinc-500"> / {stretchTarget} sessions</span>
        </p>
        <StatusPill
          met={baselineMet}
      ...
      <div className="relative mt-6">
        ...
        <div className="relative mt-2 h-5 text-xs font-medium">
          ...
          <span className={`absolute right-0 ${stretchMet ? 'text-cyan-300' : 'text-cyan-600'}`}>
  ...
  return (
    <span
      className={`rounded-full px-2.5 py-1 text-xs font-semibold ${
        met ? 'bg-emerald-500/15 text-emerald-300' : 'bg-amber-500/15 text-amber-300'
      }`}
    >
...
function RealWorkoutCadence({ cadence }: { cadence: WeeklyGoals['realWorkoutCadence'] }) {
  const latest = cadence.latestRealWorkout;
  const isCritical = cadence.status === 'critical';
  const isWarning = cadence.status === 'warning';
  const days = cadence.daysSinceRealWorkout == null ? 'No data' : cadence.daysSinceRealWorkout;
  const label =
    cadence.daysSinceRealWorkout == null
      ? 'No real workout found'
      : cadence.daysSinceRealWorkout === 0
        ? 'Real workout today'
        : cadence.daysSinceRealWorkout === 1
          ? '1 day since real workout'
          : `${cadence.daysSinceRealWorkout} days since real workout`;
  const statusText = isCritical ? 'Overdue' : isWarning ? 'Due today' : 'On cadence';

  return (
    <div
      className={`rounded-xl border p-4 ${
        isCritical
          ? 'border-red-500/70 bg-red-950/35 shadow-[0_0_0_1px_rgba(239,68,68,0.2)]'
          : isWarning
    ...
    >
        ...
        <div>
          <p
            className={`text-xs font-semibold uppercase tracking-[0.18em] ${
              isCritical ? 'text-red-300' : isWarning ? 'text-amber-300' : 'text-emerald-300'
            }`}
          >
            Real workout cadence
          </p>
      ...
      <div className="mt-4 flex items-end gap-3">
        <p
          className={`text-5xl font-semibold tabular-nums ${
            isCritical ? 'text-red-200' : isWarning ? 'text-amber-200' : 'text-emerald-200'
          }`}
        >
          {days}
        </p>
        {cadence.daysSinceRealWorkout != null && (
          <p className="pb-2 text-sm text-zinc-400">
            {cadence.daysSinceRealWorkout === 1 ? 'day' : 'days'}
          </p>
        )}
      </div>
      ...
      {latest == null ? (
        <p className="mt-3 text-xs leading-5 text-zinc-400">
          Workouts over {minutes(cadence.minimumDurationSeconds)} minutes count. Snack workouts do
          not reset this.
        </p>
      ) : (
        <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-zinc-400">
          <span className="font-medium text-zinc-300">{latest.name}</span>
          <span>
            {formatUsDate(latest.start, {
              weekday: 'short',
              month: 'short',
              day: 'numeric',
            })}{' '}
            {formatUsTime(latest.start)}
          </span>
          <span>{formatDuration(latest.durationSeconds)}</span>
        </div>
      )}
    </div>
  );
...
export default function GoalsPage() {
  ...
  const primaryGoalsMet =
    ...
    data.realWorkoutCadence.status !== 'critical';
  const liftingWorkouts = data.workouts.filter((workout) => workout.category === 'lifting');
  const cardioWorkouts = data.workouts.filter((workout) => workout.category === 'cardio');
  const flexibilityWorkouts = data.workouts.filter((workout) => workout.category === 'flexibility');
  const remainingLifts = Math.max(0, data.lifting.targetSessions - data.lifting.qualifyingSessions);
  const remainingElevatedMinutes = Math.max(
    0,
    minutes(data.cardio.targetSeconds - data.cardio.elevatedHeartRateSeconds),
  );
  const remainingFlexibility = Math.max(
    0,
    data.flexibility.targetSessions - data.flexibility.qualifyingSessions,
  );
    ...
    <div className="space-y-6">
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_22rem]">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-400">
              {data.realWorkoutCadence.status === 'critical'
                ? 'Snack workouts are still logged below, but they do not reset the every-other-day real-workout target.'
                : primaryGoalsMet
          ...
          <RealWorkoutCadence cadence={data.realWorkoutCadence} />
        </div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <section className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">Sessions of at least 20 minutes count.</p>
          </div>
              ...
              liftingWorkouts.map((workout) => (
                ...
                >
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-zinc-200">{workout.name}</p>
                  ...
                  <span
                    className={`shrink-0 text-sm tabular-nums ${
                      workout.qualifiesForLifting ? 'text-emerald-300' : 'text-amber-300'
                    }`}
                  >
                    {formatMinutesAndSeconds(workout.durationSeconds)}
                    {!workout.qualifiesForLifting && (
                      <span className="ml-1 text-xs text-zinc-500">under 20 min</span>
                    )}
                  </span>
                </Link>
              ))
        ...
        <section className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-sm font-medium text-zinc-400">Elevated heart rate</p>
          ...
          <div className="mt-5 rounded-lg border border-zinc-800 bg-zinc-950/60 p-4">
            <div className="flex items-baseline justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-wide text-zinc-500">Cardio workout time</p>
            ...
            <p className="mt-2 text-xs leading-5 text-zinc-500">
              All workout types except lifting and flexibility. This is the schedule-based proxy;
              measured heart-rate time above is the primary cardio goal.
            </p>
          </div>
          ...
          <details className="mt-4 rounded-lg border border-zinc-800 p-3 text-xs text-zinc-500">
            ...
            <p className="mt-2 leading-5">
              ...
              from {data.elevatedHeartRate.calibrationDays} daily values. Sensor gaps over{' '}
              {data.elevatedHeartRate.maximumSampleGapSeconds} seconds are excluded.
            </p>
          </details>
        </section>
      </div>
          ...
          {flexibilityWorkouts.length === 0 ? (
            ...
            flexibilityWorkouts.map((workout) => (
              ...
              >
                ...
                <span
                  className={`shrink-0 text-sm tabular-nums ${
                    workout.qualifiesForFlexibility ? 'text-emerald-300' : 'text-amber-300'
                  }`}
                >
                  {formatMinutesAndSeconds(workout.durationSeconds)}
                  {!workout.qualifiesForFlexibility && (
                    <span className="ml-1 text-xs text-zinc-500">not over 20 min</span>
                  )}
                </span>
              </Link>
            ))
      ...
      <section className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5">
        <div className="flex flex-wrap items-end justify-between gap-2">
          <div>
            <h3 className="font-semibold text-zinc-100">Cardio contribution</h3>
            <p className="mt-1 text-sm text-zinc-500">
              Workouts included in the rolling cardio-time proxy.
            </p>
          </div>
          <p className="text-xs text-zinc-500">
            {minutes(data.cardio.heartRateTrackedSeconds)} min with usable heart-rate coverage
          </p>
        </div>
          ...
          {cardioWorkouts.map((workout) => (
            ...
            >
              ...
              <div className="mt-2 flex justify-between text-xs text-zinc-500">
                ...
                <span>{minutes(workout.elevatedHeartRateSeconds)} min elevated</span>
              </div>
            </Link>
          ))}
        </div>
